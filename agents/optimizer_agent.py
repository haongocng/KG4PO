...
class OptimizerAgent:
    ...
    def optimize(self, current_prompt: str, metrics: dict, failed_cases: list) -> dict:
        ...
        OBJECTIVE:
        Improve the system prompt so that the recommender produces more relevant movie suggestions.
        ...
        - Focus on improving reasoning and recommendation constraints.
        ...
        if not failed_cases:
            ...
            for idx, fc in enumerate(failed_cases[:5]):
                ...
                failed_cases_str += f"   + User history: {fc.get('session_raw', [])}\n"
                failed_cases_str += f"   + Ground truth (Actual next item): {fc.get('target_raw','')}\n"
                failed_cases_str += f"   + System's top predictions: {fc.get('predictions', [])[:10]}\n\n"
